...
for i=1:size(model_names, 2)
    ...
    ADL_k_indexes{i} = k_sets_indexes;
    
    %models(i,:).name = model_names{i};
    for validation_set_index = 1:number_k_sets
        fprintf('-Building fold %d..\n',validation_set_index);
        ...
        models(i,validation_set_index).name = model_names{i};

    end
    clear modelfile processed_data k_sets k_sets_indexes 
    close all;
    fclose all;
end
...
%         temp_model = [models(:,validation_set_index)];
...
for validation_set_index = 1:number_k_sets
    ...
    for i=1:size(model_names, 2)
        ...
        k_sets_indexes = ADL_k_indexes{i};
        ...
        for val_file_index = k_sets_indexes(validation_set_index,:)
            ...
            ValidateTrial( temp_model, single_val_trial_data, validation_file_name, 1 );
        end
