...
% COMPUTE THE DISTANCE BETWEEN THE ACTUAL DATA AND THE MODEL
% with dynamic time warping to overcome the different lengths of the data
i = 1:1:3;
...
% % DISPLAY THE RESULTS
% % display the optimal paths computed by DTW - gravity
% figure,
%     scatter(pathG(:,1),pathG(:,2),10,'k','filled');
%     title('Optimal warping path');
% % display the effect of warping on the accelerations
% % compact view of the output of DTW
% % comparison between the acceleration patterns (x axis only)
% gravity = gravity';
% for i=1:1:norm_factorG
%     norm_tG(i) = gravity(1,pathG(i,1));
%     norm_rG(i) = reference_G(1,pathG(i,2));
% end
% body = body';
% for i=1:1:norm_factorB
%     norm_tB(i) = body(1,pathB(i,1));
%     norm_rB(i) = reference_B(1,pathB(i,2));
% end
% figure,
%     % gravity
%     % before DTW
...
%     plot(gravity(1,:),'b');
%     hold on;
%     plot(reference_G(1,:),'r');
%     title('acceleration curves - pre DTW');
%     % after DTW
%     subplot(2,2,3);
%     plot(norm_factorG:-1:1,norm_tG,'b');
%     hold on;
%     plot(norm_factorG:-1:1,norm_rG,'r');
...
%     plot(body(1,:),'b');
%     hold on;
%     plot(reference_B(1,:),'r');
%     title('acceleration curves - pre DTW');
%     % after DTW
%     subplot(2,2,4);
%     plot(norm_factorB:-1:1,norm_tB,'b');
%     hold on;
%     plot(norm_factorB:-1:1,norm_rB,'r');
%     title('acceleration curves - post DTW');